...
jQuery(function ($) {
    ...
    $('.form-date-search').each(function () {
        ...
        var options = {
            ...
            minDate: today,
            opens: planner.rtl ? 'right' : 'left',
            locale: {
                format: "YYYY-MM-DD",
                direction: planner.rtl ? 'rtl' : 'ltr',
        ...
        check_in_out.daterangepicker(options,
            function (start, end, label) {
                check_in_input.val(start.format(planner.date_format));
                check_in_render.html(start.format(planner.date_format));
                check_out_input.val(end.format(planner.date_format));
                check_out_render.html(end.format(planner.date_format));
            });
    ...
    $('.form-date-search-hotel').each(function () {
        ...
        check_in_out.daterangepicker(options).on('apply.daterangepicker',
            function (ev, picker) {
                ...
                check_in_input.val(picker.startDate.format(planner.date_format));
                check_in_render.html(picker.startDate.format(planner.date_format));
                check_out_input.val(picker.endDate.format(planner.date_format));
                check_out_render.html(picker.endDate.format(planner.date_format));
    ...
    $('.bravo-form-login [type=submit]').click(function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.two_factor) {
                    return window.location.href = planner.url + '/two-factor-challenge';
                }
                ...
                if (redirect.trim('/')) {
                    window.location.href = planner.url_root + form.find('input[name=redirect]').val();
                } else {
    ...
    $(".smart-search .smart-search-location").each(function () {
        ...
        var options = {
            url: url ? url : planner.url + '/location/search/searchForSelect2',
    ...
    $('.btn-submit-enquiry').click(function (e) {
        ...
        $.ajax({
            url: planner.url + '/booking/addEnquiry',
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        if (res.data) {
                            ...
                            if (count > 5) {
                                plannerApp.showError('Maximum upload 6 pictures');
                            } else {
...
jQuery(function ($) {
    ...
    if (planner.pusher_api_key && planner.pusher_cluster) {
        var pusher = new Pusher(planner.pusher_api_key, {
            encrypted: true,
            cluster: planner.pusher_cluster
        });
    }
    ...
    if (planner.isAdmin > 0 && planner.pusher_api_key) {
        var channel = pusher.subscribe('admin-channel');
        channel.bind('App\\Events\\PusherNotificationAdminEvent', callback);
    }

    if (planner.currentUser > 0 && planner.pusher_api_key) {
        var channelPrivate = pusher.subscribe('user-channel-' + planner.currentUser);
