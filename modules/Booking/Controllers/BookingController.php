...
class BookingController extends \App\Http\Controllers\Controller
{
    ...
    protected function validateDoCheckout()
    {
        ...
        if (
            ...
            !auth()
                ->user()
                ->hasVerifiedEmail() &&
            setting_item('enable_verify_email_register_user') == 1
        ) {
    ...
    public function doCheckout(Request $request)
    {
        ...
        if ($request->has('payment_gateway') && $request->payment_gateway == 'peach_payment') {
            $amount = (float) $booking->total;
            session_start();
            $_SESSION['amount'] = $amount;
            $_SESSION['return_url'] = 'booking/' . $booking->code;
    ...
    {
        if (!is_admin() and $booking->vendor_id != auth()->id() and $booking->customer_id != auth()->id()) {
            abort(404);
        }
    ...
    public function peachCheckout(Request $request)
    {
        $entityId = config('peach-payment.entity_id');
        session_start();
        $amount = $_SESSION['amount'];
        $return_url = config('peach-payment.domain') . '/' . $_SESSION['return_url']; //'after-main-domain/route/sub-route/?PeachPaymentOrder=OID123456789'; // Optional
        ...
        return view('Booking::frontend/gateways/peach', compact('entityId', 'checkoutId'));
    }
}
